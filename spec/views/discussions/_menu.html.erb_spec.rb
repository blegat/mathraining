...
describe "discussions/_menu.html.erb", type: :view, discussion: true do
  ...
  context "if the user has two discussions" do
    ...
    let!(:link21) { Link.create(user: user2, discussion: discussion12) }
    let!(:link23) { Link.create(user: user2, discussion: discussion23) }
    let!(:link32) { Link.create(user: user3, discussion: discussion23) }
    let!(:link31) { Link.create(user: user3, discussion: discussion31) }
