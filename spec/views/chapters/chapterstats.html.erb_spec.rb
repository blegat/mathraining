...
describe "chapters/chapterstats.html.erb", type: :view, chapter: true do
  ...
  context "if the user is an admin" do
    ...
    it "renders the statistics correctly" do
      ...
      expect(rendered).to have_no_link("0%", href: chapter_path(chapter1, :type => 5, :which => question11_offline.id))
      expect(rendered).to have_link("67%", href: chapter_path(chapter1, :type => 5, :which => question12.id))
      expect(rendered).to have_link(chapter2.name, href: chapter_path(chapter2))
      expect(rendered).to have_selector("td", text: "25%")
      expect(rendered).to have_link("14%", href: chapter_path(chapter2, :type => 5, :which => question21.id))
      expect(rendered).to have_link("0%", href: chapter_path(chapter2, :type => 5, :which => question22.id))
  ...
  context "if the user has not solved prerequisites" do
    ...
    it "renders the statistics correctly" do
      ...
      expect(rendered).to have_link(chapter2.name, href: chapter_path(chapter2))
      expect(rendered).to have_selector("td", text: "25%")
      expect(rendered).to have_no_link("14%", href: chapter_path(chapter2, :type => 5, :which => question21.id))
      expect(rendered).to have_selector("td", text: "14%")
      expect(rendered).to have_no_link("0%", href: chapter_path(chapter2, :type => 5, :which => question22.id))
