...
class Record < ActiveRecord::Base
  def self.update
    ...
    while(lundiencours <= lundidernier)
      ...
      r.number_solved = Solvedexercise.where("correct = ? AND updated_at >= ? AND updated_at < ?", true, lundiencours.to_time.to_datetime, lundiapres.to_time.to_datetime).count + Solvedqcm.where("correct = ? AND updated_at >= ? AND updated_at < ?", true, lundiencours.to_time.to_datetime, lundiapres.to_time.to_datetime).count
      r.complete = false
      r.save
